...
function loginError(res, msg) {
  // See the matching comment in zerodot-start.js's fail(): the clean path
  // avoids cleanUrls' redirect stripping the query string.
  redirect(res, `/pages/login?error=${encodeURIComponent(msg)}`);
}
...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    redirect(res, `/pages/login?connected=1&next=${encodeURIComponent(next)}`);
  } catch (err) {
